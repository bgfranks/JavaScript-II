...
let fullNames = [];
let names = runners.forEach(fullName => {
...
let firstNamesAllCaps = runners.map(person => {
...
let runnersLargeSizeShirt = runners.filter(size => {
...
let ticketPriceTotal = runners.reduce((total, people) => {
...
// Problem 1
let CompanyNameDonation = runners.map(company => {
  return `${company.company_name} ${company.donation}`;
});
console.log(CompanyNameDonation);

// Problem 2
let highDonation = runners.filter(donation => {
  return donation.donation > 200;
});
console.log(highDonation);

// Problem 3
let emailID = [];
let emails = runners.forEach(email => {
  emailID.push(`${email.id} ${email.email}`);
});
console.log(emailID);
